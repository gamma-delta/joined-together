...
void main() {
    ...
    vec4 noise = openSimplex2SDerivativesPart(vec3(gl_FragCoord.x / 80.0 + 100.0, gl_FragCoord.y / 5.0 + 5000.0, time * 0.03));
    float modify = clamp(abs(noise.w), 0.0, 0.5);
    float vary = sin(time) * 0.3 + 1.0;
    float opacity = pow(outCol.a, modify * vary / (outCol.a + 0.001));
